              ...
              ∀ (central : Rat),
                ∀ (periph : Rat),
                  depot_rate ka depot +ₚ central_rate ka ke k12 k21 depot central periph +ₚ
                    ...
                    -ke *ₚ central :=
  by (simp_all only [depot_rate, pyMul_rat, neg_mul, central_rate, pySub_rat, pyAdd_rat, periph_rate]; grind only)
...
theorem distribution_conserves :
  ...
  by simp_all only [pyMul_rat, neg_mul, pyAdd_rat, pySub_rat, add_add_sub_cancel, neg_add_cancel, Int.cast_zero, implies_true]
            ...
            ∀ (central : Rat),
              ∀ (periph : Rat),
                depot_rate ka depot +ₚ central_rate ka (0 : Int) k12 k21 depot central periph +ₚ
                    periph_rate k12 k21 central periph =
                  (0 : Int) :=
  by (simp_all only [depot_rate, pyMul_rat, neg_mul, central_rate, Int.cast_zero, zero_mul, pySub_rat, sub_zero, pyAdd_rat, periph_rate]; grind only)
...
def step_mass_balance := fun (ka : Rat) ↦ fun (ke : Rat) ↦ fun (k12 : Rat) ↦ fun (k21 : Rat) ↦ fun (depot : Rat) ↦
  ...
  have ht_1 : new_depot +ₚ new_central +ₚ new_periph = depot +ₚ central +ₚ periph -ₚ ke *ₚ central *ₚ dt := by (simp_all only [pyAdd_rat, pyMul_rat, pySub_rat]; simp_all? (config := { zetaDelta := true }) [taste_ingr]; grind only)
  ()
...
theorem depot_nonincreasing :
    ∀ (ka : Rat), ∀ (depot : Rat), ka ≥ (0 : Int) → depot ≥ (0 : Int) → depot_rate ka depot ≤ (0 : Int) := by simp_all only [Int.cast_zero, ge_iff_le, depot_rate, pyMul_rat, neg_mul, Left.neg_nonpos_iff, mul_nonneg, implies_true]
