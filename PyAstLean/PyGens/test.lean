...
noncomputable def sigmoid := fun (x : Rat) ↦ (1.0 : Rat) /ₚ ((1.0 : Rat) +ₚ Libraries.math.pyMathExpR (-x))

def predict := fun (x : List Rat) ↦ fun (w1 : List (List Rat)) ↦ fun (b1 : List Rat) ↦
  fun (w2 : List (List Rat)) ↦ fun (b2 : List Rat) ↦
...
def mean_squared_error := fun (xs : List (List Rat)) ↦ fun (ys : List Rat) ↦ fun (w1 : List (List Rat)) ↦
  fun (b1 : List Rat) ↦ fun (w2 : List (List Rat)) ↦ fun (b2 : List Rat) ↦
  Id.run
    (do
...
def main' :=
  ((do
      ...
      let mut w1 := [[(0.5 : Rat), -(0.4 : Rat)], [(0.9 : Rat), (1.0 : Rat)]]
      let mut b1 := [(0.1 : Rat), -(0.2 : Rat)]
      let mut w2 := [[(0.7 : Rat), -(0.8 : Rat)]]
      let mut b2 := [(0.3 : Rat)]
      let mut lr := (0.5 : Rat)
      ...
      for epoch in (PyAstLean.pyRange epochs)do
        for i in (PyAstLean.pyRange (PyAstLean.pyLen xs))do
          ...
          let mut d_out := (out -ₚ y) *ₚ out *ₚ ((1.0 : Rat) -ₚ out)
          let mut d_h0 := d_out *ₚ w2⦋(0 : Int)⦌⦋(0 : Int)⦌ *ₚ h0 *ₚ ((1.0 : Rat) -ₚ h0)
          let mut d_h1 := d_out *ₚ w2⦋(0 : Int)⦌⦋(1 : Int)⦌ *ₚ h1 *ₚ ((1.0 : Rat) -ₚ h1)
      ...
      for i in (PyAstLean.pyRange (PyAstLean.pyLen xs))do
        ...
        let mut label := if decide (p > (0.5 : Rat)) then (1 : Int) else (0 : Int)
