...
def applyMutation (env : Env) (value : Json) : Env :=
  ...
  | some func =>
      ...
      | some attr, some cname =>
          ...
          let elemFrom (i : Nat) : PyType := (args[i]?).elim .unknown (typeOfExpr env)
          let learned : PyType := match attr with
            | "append" | "add" | "insert" => .list (elemFrom (if attr == "insert" then 1 else 0))
            | "extend" => match args[0]?.elim .unknown (typeOfExpr env) with
                          | .list e => .list e
                          | _ => .unknown
            | _ => .unknown
...
def applyStmt (env : Env) (s : Json) : Env :=
  ...
  | some "Assign" =>
      ...
      | some target, some value =>
          ...
          | some name => learn env name (typeOfExpr env value)
          ...
          | none =>
              if nodeTypeOf target == some "Subscript" then
                ...
                | some cname =>
                    let vt := typeOfExpr env value
                    let learned := match env.get? cname |>.getD .unknown with
                      | .dict _ _ => .dict ((getField target "slice").elim .unknown (typeOfExpr env)) vt
                      | _ => .list vt
  ...
  | some "AugAssign" =>
      ...
      | some name, some value => learn env name (arith (env.get? name |>.getD .unknown) (typeOfExpr env value))
      | _, _ => env
  | some "For" =>
      match (getField s "target").bind nameId?, getField s "iter" with
      | some name, some iter => learn env name (typeOfExpr env iter).elemType
      | _, _ => env
  -- `xs.append(v)` / `xs.add(v)` teaches that `xs` holds values of `v`'s type.
  | some "Expr" =>
      match getField s "value" with
      | some value => applyMutation env value
      | none => env
  | _ => env
...
with the enclosing scope so a nested def's captures start typed. -/
partial def inferFunction (outer : Env) (fn : Json) : Env := Id.run do
  ...
  for _ in [0:8] do
    let next := stmts.foldl applyStmt env
...
/-- Stamp `_ty` (an annotation node) on a target if we know a fully-determined type for it. Tuple
targets stamp each element. -/
...
un-hinted param). An explicit annotation always wins, so we never overwrite one. -/
...
/-- Infer types for `fn` (seeded by `outer`), stamp its params and every binder in its body, and
recurse into nested defs with the now-known environment. -/
partial def stampFunction (outer : Env) (fn : Json) : Json :=
  let env := inferFunction outer fn
  let fn := stampParams env fn
  match fn.getObjValAs? (Array Json) "body" with
  | .ok body => fn.setObjVal! "body" (Json.arr (body.map (stampStmt env)))
  | _ => fn

/-- Stamp one statement: its target, its nested blocks, and any nested def. -/
partial def stampStmt (env : Env) (s : Json) : Json :=
  if nodeTypeOf s == some "FunctionDef" then stampFunction env s
  else Id.run do
    let mut s := s
    match nodeTypeOf s with
    ...
    for f in #["body", "orelse", "finalbody"] do
      if let .ok elems := s.getObjValAs? (Array Json) f then
        s := s.setObjVal! f (Json.arr (elems.map (stampStmt env)))
    if let .ok handlers := s.getObjValAs? (Array Json) "handlers" then
      let handlers := handlers.map fun h =>
        match h.getObjValAs? (Array Json) "body" with
        | .ok elems => h.setObjVal! "body" (Json.arr (elems.map (stampStmt env)))
        | _ => h
...
/-- Stamp `_ty` across one top-level node. The driver sends one statement per request, so this
handles a `FunctionDef`, a `ClassDef` (each method), or a `Module` (a mutual group); anything else
is returned unchanged. Each top-level function starts with an empty environment — cross-function
flow is P2's job. -/
partial def stampNode (s : Json) : Json :=
  match nodeTypeOf s with
  | some "FunctionDef" => stampFunction {} s
  | some "ClassDef" =>
      match s.getObjValAs? (Array Json) "body" with
      | .ok methods => s.setObjVal! "body" (Json.arr (methods.map fun m =>
          if nodeTypeOf m == some "FunctionDef" then stampFunction {} m else m))
      | _ => s
  | some "Module" =>
      match s.getObjValAs? (Array Json) "body" with
      | .ok body => s.setObjVal! "body" (Json.arr (body.map stampNode))
      | _ => s
  | _ => s
